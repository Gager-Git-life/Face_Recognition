 ...
 * @version: 
 * @Author: Gager
 * @Date: 2020-11-26 15:51:31
 * @LastEditors: sueRimn
 * @LastEditTime: 2020-12-04 15:13:23
 */
...
MobilefaceNet::MobilefaceNet(const std::string &mnn_path, int num_thread_){

    //加载模型
    mobileface_interpreter = std::shared_ptr<MNN::Interpreter>(MNN::Interpreter::createFromFile(mnn_path.c_str()));
    // 配置调度
    ...
    // 创建会话
    mobileface_session = mobileface_interpreter->createSession(config);
    input_tensor = mobileface_interpreter->getSessionInput(mobileface_session, nullptr);
...
MobilefaceNet::~MobilefaceNet(){
    ...
    mobileface_interpreter->releaseModel();
    mobileface_interpreter->releaseSession(mobileface_session);
}   


cv::Mat MobilefaceNet::Get_Resize_Croped_Img(cv::Mat frame, cv::Point pt1, cv::Point pt2, cv::Point &s_point, cv::Size &croped_wh){
...
void MobilefaceNet::GetFeature(const cv::Mat &frame, vector<float> &feature){

    auto nchw_data   = nchw_Tensor->host<float>();
    auto nchw_size   = nchw_Tensor->size();
    ::memcpy(nchw_data, frame.data, nchw_size);

    auto input_tensor  = mobileface_interpreter->getSessionInput(mobileface_session, nullptr);
    input_tensor->copyFromHostTensor(nchw_Tensor);

    // run network
    mobileface_interpreter->runSession(mobileface_session);
    // get output data
    string output_tensor_name = "output0"; 
    MNN::Tensor *tensor_feature = mobileface_interpreter->getSessionOutput(mobileface_session, output_tensor_name.c_str());
